...
def get_dicom_series_fetch_rows_query(db, select_columns, qnode, endpoint_tablename, log):
        ...
        for column in select_columns:
            ...
            if uniquename in DB_MAP.file_dicom_column_map.keys():
                ...
                if DB_MAP.get_column_info(uniquename).tablename == "file":
                    dicom_select_columns.append(
                        DB_MAP.file_dicom_column_map[uniquename].metadata_column.label(uniquename)
                    )
                    ...
                    raise ColumnNotFound(
                        f"Could not find column: {dicom_uniquename} in either dicom_series, or in the foreign array preselects"
                    )
            else:
    ...
    count_subquery = (
        db.query(dicom_id_alias).filter(dicom_id_alias.in_(dicom_preselect_query)).subquery("rows_to_count_dicom")
    )
